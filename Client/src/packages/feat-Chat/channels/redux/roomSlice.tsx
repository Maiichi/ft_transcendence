...
export const roomSlice = createSlice({
  ...
  reducers: {
    ...
    removeMemberFromRoom: (state, action: PayloadAction<any>) => {
        ...
        const isMembershipFound: I_Room | undefined = state.memberships.find((membership: I_Room) => membership.id === roomId)
        console.log('newState (isMem) ===', JSON.stringify(isMembershipFound));
        if (isMembershipFound)
        {
          console.log('here');
          const memberIndex = isMembershipFound.members.findIndex((member) => member.user.intraId == userId)
