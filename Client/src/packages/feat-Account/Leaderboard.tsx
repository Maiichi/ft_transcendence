...
const Leaderboard = () => {
	...
	return (
		<Root $primary={true}>
			...
			{ state.isLoading? <Loading/>: state.leaderboard? 
			<Players>
				{state.leaderboard.map((player:any, index:number) => (
				<Player >
					<ListItemAvatar>
					...
					/>
					<ListItemIcon>
						<Button onClick={() => sendGameRequist(player.name)}><SportsCricket /></Button>
						<Button onClick={() => navigate('/chat')}><Chat /></Button>
						<Button onClick={() => sendFriendRequist(player.name)}><PersonAdd /></Button>
					</ListItemIcon>
				</Player>
			))}
			</Players>
